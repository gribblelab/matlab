% ------------------------------------------------------------------------
% plg_zip_load: - modified from BKIN's zip_load
%                 by Paul Gribble, April 15 2019
%               - uses plg_unzip() to unzip .zip files rather
%                 than MATLAB's built-in java-based routine
%                 (which is slow)
%               - checks to see if the temporary directory
%                 unzipToName exists before attempting to delete it
%                 (this fixes an occasional error that occurs when for
%                 some unknown reason unzipToName doesn't actually exist
...
function c3dstruct = plg_zip_load(varargin)
...
%        unzip(zipfiles{x}, unzipToName)
        plg_unzip(zipfiles{x}, unzipToName);
        cd (unzipToName)
